...
本分支引入了**多 Agent 协作架构**，基于 CoPaw 框架实现群聊场景下的智能体协同工作。
...
| Agent | 角色定位 | 响应场景 | 核心能力 |
|-------|----------|----------|----------|
| **软件工程师助手** | 技术顾问 | 代码、Bug、架构 | 代码审查、性能优化、故障排查 |
| **产品经理助手** | 需求分析 | 需求、工期、方案 | 需求评估、方案设计、优先级排序 |
| **招聘专员机器人** | HR助手 | 招聘、面试、内推 | 简历筛选、面试安排、流程跟进 |

**协作机制：**
- **关键词路由**：根据消息内容自动分发给对应 Agent
- **共享记忆**：所有 Agent 共享群聊上下文和知识库
- **状态感知**：Agent 之间了解彼此的工作状态，避免冲突
- **协同响应**：复杂问题可由多个 Agent 联合回答
